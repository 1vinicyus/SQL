...
2- Relatório de horas
3- Horas planejadas | Horas trabalhadas | Diferença

PROCEDURES DE GESTÃO (substituem as Views):
1- Fechar o mês de um cliente
3- Reabrir um mês fechado
4- Corrigir uma hora já lançada
*/
...
-- ============================================
-- TRIGGER 4 (NOVA): IMPEDIR HORA EM MÊS FECHADO
-- ============================================
DELIMITER $$
...
-- ============================================
-- PROCEDURE 1: PLANEJADO X REALIZADO
-- ============================================
DELIMITER $$
DROP PROCEDURE IF EXISTS sp_planejado_realizado $$
CREATE PROCEDURE sp_planejado_realizado(
...
BEGIN
    ...
    FROM (
        ...
        FROM planejamento WHERE data_planejada BETWEEN p_data_inicio AND p_data_fim
        UNION
        SELECT cpf_colaborador, data_registro AS data_referencia, id_tarefa
        FROM registroHora WHERE data_registro BETWEEN p_data_inicio AND p_data_fim
    ) AS dados
    ...
    ) AS p ON p.cpf_colaborador = dados.cpf_colaborador AND p.data_planejada = dados.data_referencia AND p.id_tarefa = dados.id_tarefa
    LEFT JOIN (
    ...
    ) AS r ON r.cpf_colaborador = dados.cpf_colaborador AND r.data_registro = dados.data_referencia AND r.id_tarefa = dados.id_tarefa
    ORDER BY dados.data_referencia, c.nome_colaborador;
END $$
DELIMITER ;


-- ============================================
-- PROCEDURE 2: RELATÓRIO DE HORAS TRABALHADAS
-- ============================================
DELIMITER $$
DROP PROCEDURE IF EXISTS sp_relatorio_horas $$
CREATE PROCEDURE sp_relatorio_horas(
    IN p_data_inicio DATE,
    IN p_data_fim DATE
)
BEGIN
    SELECT
        c.cpf_colaborador,
        c.nome_colaborador,
        COUNT(DISTINCT r.id_tarefa) AS quantidade_tarefas,
        COUNT(r.id_registro) AS quantidade_registros,
        SEC_TO_TIME(SUM(TIME_TO_SEC(r.hora_trabalhada))) AS total_horas_trabalhadas
    FROM registroHora r
    INNER JOIN colaborador c ON c.cpf_colaborador = r.cpf_colaborador
    WHERE r.data_registro BETWEEN p_data_inicio AND p_data_fim
    GROUP BY c.cpf_colaborador, c.nome_colaborador
...
-- ============================================
-- PROCEDURE 3: PLANEJADO X TRABALHADO X DIFERENÇA
-- ============================================
DELIMITER $$
DROP PROCEDURE IF EXISTS sp_comparativo_horas $$
CREATE PROCEDURE sp_comparativo_horas(
    IN p_data_inicio DATE,
    IN p_data_fim DATE
)
BEGIN
    ...
    LEFT JOIN (
        ...
        FROM planejamento WHERE data_planejada BETWEEN p_data_inicio AND p_data_fim
        GROUP BY cpf_colaborador, data_planejada
    ) AS p ON p.cpf_colaborador = dados.cpf_colaborador AND p.data_planejada = dados.data_referencia
    LEFT JOIN (
        SELECT cpf_colaborador, data_registro, SUM(TIME_TO_SEC(hora_trabalhada)) AS total_trabalhado
        FROM registroHora WHERE data_registro BETWEEN p_data_inicio AND p_data_fim
        GROUP BY cpf_colaborador, data_registro
    ) AS r ON r.cpf_colaborador = dados.cpf_colaborador AND r.data_registro = dados.data_referencia
    ORDER BY dados.data_referencia, c.nome_colaborador;
END $$
DELIMITER ;
...
-- ============================================
-- PROCEDURE 4 (NOVA): FECHAR O MÊS DE UM CLIENTE
-- ============================================
DELIMITER $$
...
-- ============================================
-- PROCEDURE 5 (NOVA): REABRIR UM MÊS FECHADO
-- ============================================
DELIMITER $$
...
-- ============================================
-- PROCEDURE 6 (NOVA): CORRIGIR UMA HORA JÁ LANÇADA
-- ============================================
DELIMITER $$
